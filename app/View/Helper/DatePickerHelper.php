...
class DatePickerHelper extends FormHelper {
    ...
    */    
    function picker($fieldName, $options = array()) {
		...
		$image = $this->Html->image('cal_img.gif');
		echo $image;
        $options['after'] = $this->Html->link($image, '#', array('onClick'=>"return showCalendar('".$htmlAttributes['id']."', '".$this->format."'); return false;"), null, false);
